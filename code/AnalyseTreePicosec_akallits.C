...
int AnalyseTreePicosec(int runNo=15, int poolNo=2, int draw=0, double threshold = 20.0, double peTh = 22.0, string filetype = "")
{
...
//      tlastgoodpeaks = tnowgoodpeaks ;
 ...
 /// 2nd IF DRAW
    ...
    //double DTI2 = 50.;
    /*double DTI2 = 2.;
    int nint = TMath::FloorNint(DTI2/dt)+1;
    double intgr = IntegratePulse(maxpoints,idamplC,iampl,dt,nint*dt);
    TGraph* graph23 = new TGraph(maxpoints,ptime,iampl);
    graph22 = new TGraph(maxpoints,ptime, dampl[ci]);
    //graph23->SetMarkerColor(clr[ci+15]);
    //graph23->SetLineColor(clr[ci+15]);
    //graph23->SetFillColor(0);
    //graph23->SetLineWidth(2);
    //graph23->SetLineStyle(7);
    //graph22 = new TGraph(maxpoints,ptime, dampl[ci]);
    integralh->SetMarkerColor(clr[ci+15]);
    integralh->SetLineColor(clr[ci+15]);
    integralh->SetFillColor(0);
    integralh->SetLineWidth(2);
    integralh->SetLineStyle(7);
    sprintf(cname,"Event %d - Integral %g of C%d\n",evNo,nint*dt,ci+1);
    integralh->SetTitle(cname);
    integralh->Draw("apl");
    //graph22->SetMarkerColor(1);
    ...
    */
    ...
    //cout<<"N integration points = "<<N_INTEGRATION_POINTS<<endl;
    //double DTI2 = 2.;  ///default
    double DTI2 = 200.; //time window for integration in ns
...
/*
    ...
    mg->Draw("ap");
      ...
      h2->GetYaxis()->SetTitle("[V]");
      h3->GetXaxis()->SetTitle("[ns]");
      h3->GetYaxis()->SetTitle("[V]");
	    evdcanv[ci]->Modified();
	    evdcanv[ci]->cd(2);
	    evdcanv[ci]->Update();
	  }
