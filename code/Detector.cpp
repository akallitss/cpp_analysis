...
void Detector::TimeGeneralSigmoid()
{
    int start = start_point.position - 5; 
    while(start < 0) start++;
   ...
   while(end >= waveform_y.size()) end--;
    ...
    TF1 fd_fit("fd_fit", fermi_dirac_general, x[0], x[Npoints-5], 5);
    fd_fit.SetParameters(pars[0],pars[1],pars[2],pars[3]);                              
...
//      }
    ...
    GeneralSigmoid.chisquare = fd_fit.GetChisquare();

        double para[6];

    for(int i=0; i<6;i++){
    GeneralSigmoid.parameters[i] = fd_fit.GetParameter(i);
    para[i]= fd_fit.GetParameter(i);
    }

    GeneralSigmoid.degrees_freedom = fd_fit.GetNDF();
    GeneralSigmoid.chisquare = fd_fit.GetChisquare()/fd_fit.GetNDF();
    GeneralSigmoid.fit_func = fd_fit;
    GeneralSigmoid.failed = GeneralSigmoid.chisquare < 1000 ? 0 : 1;

    //  Sigmoid.timepoint=Sigmoid.parameters[1]-(TMath::Log((Sigmoid.parameters[0]/(0.2*global_maximum.y-Sigmoid.parameters[3]))-1)/Sigmoid.parameters[2]);
    // Sigmoid.timepoint=1-(TMath::Log(((Sigmoid.parameters[0]/(0.2*global_maximum.y-Sigmoid.parameters[4]))^(1/Sigmoid.parameters[3]))-1))/Sigmoid.parameters[4];

    double time20=0;
    double timebin=0;
    double timeval=x[0];
    
    while(time20<0.2*global_maximum.y){
        timebin++;
        timeval=x[0]+0.0001*timebin;
        time20=para[0]/TMath::Power((1+TMath::Exp(-(timeval-para[1])*para[2])),para[3]);
        if(timeval>global_maximum.x){
    ...
    GeneralSigmoid.timepoint=timeval;
